...
GLYPH_MAP = os.path.abspath(os.path.join(HERE, "..", "src", "ui", "glyph_map.h"))
...
# ASCII and Latin-1 sit at their own codepoints.
ASCII = [chr(c) for c in range(0x20, 0x7F)]
LATIN1 = [chr(c) for c in range(0xA0, 0x100)]

# Common punctuation above Latin-1, parked in the unused C1 range. The mapping
# is written to src/ui/glyph_map.h for the text helper.
EXTRAS = ["…", "–", "—", "‘", "’", "“", "”", "•", "€"]
EXTRA_BASE = 0x80

assert len(EXTRAS) <= 0x20, "EXTRAS must fit in 0x80-0x9F"

TEXT_SET = ASCII + LATIN1 + EXTRAS
...
def write_glyph_map():
    """Where each extra lives in the fonts, for the text helper."""
    lines = [
        "// Generated by tools/genfonts.py. Do not edit.",
        "#pragma once",
        "",
        ...
        *[f"    {{0x{ord(c):04X}, 0x{index_of(c):02X}}},  // {c}" for c in EXTRAS],
        "};",
        "",
        "}  // namespace ui",
        "",
    ...
    with open(GLYPH_MAP, "w") as handle:
        handle.write("\n".join(lines))
